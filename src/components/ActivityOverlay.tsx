...
export function ActivityOverlay({
  ...
  // Top panel gets roughly half the rows, minus overhead for borders/title/status
  ...
  if (entries.length === 0) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold>Sub-agents ─ {sessionName}</Text>
        <Text> </Text>
        <Text dimColor>No sub-agents spawned in this session</Text>
        <Text> </Text>
  ...
  // Detail panel: wrap prompt to ~15 lines
  ...
  return (
    <Box flexDirection="column" padding={1}>
      {/* Header */}
      <Text bold>
        Sub-agents ─ {entries.length} total │ {running} running │ {completed} completed
      </Text>
      ...
      <Box flexDirection="column">
        ...
        {visibleSlice.map((entry, i) => {
          ...
          return (
            <Box key={entry.spawnId}>
              <Text color={isSel ? 'cyan' : undefined} bold={isSel}>
                {isSel ? '› ' : '  '}
              </Text>
              <Text color={iconColor}>{icon}</Text>
              <Text color={isSel ? 'cyan' : undefined} bold={isSel}>
                {' '}{formatTime(entry.timestamp)}  {agentType} {desc}
              </Text>
      ...
      {/* Bottom panel: detail */}
      <Box flexDirection="column">
        <Text bold underline>{selected.description || 'Sub-agent detail'}</Text>
        <Text> </Text>
        <Text>
        ...
        <Text>
          ...
          <Text color={selected.status === 'completed' ? 'green' : 'yellow'}>
            {selected.status === 'completed' ? '✓ Completed' : '⟳ Running'}
          </Text>
        </Text>
        <Text>
        ...
        {resultLines.length > 0 && (
          <Box flexDirection="column" marginTop={1}>
            ...
            {resultLines.map((line, i) => (
              <Text key={i} color="green" wrap="truncate">{line}</Text>
            ))}
          </Box>
        )}
      </Box>
